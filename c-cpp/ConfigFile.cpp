	...
	std::map<std::string, std::string>& key_values)
{
	...
	while (std::getline(reader, line_content))
	{
		...
		else
		{
			...
			size_t to_left = 0;
			for (to_left = equal_sign_index-1; line_content[to_left] == ' ';) { --to_left; }
			std::string key = line_content.substr(no_space_index, to_left+1);
			size_t to_right = 0;
			for (to_right = equal_sign_index+1; line_content[to_right] == ' ';) { ++to_right; }
			std::string value = line_content.substr(to_right, line_content.length());
...
config_file_rw::ConfigFile::ConfigFile(const std::string& fileName)
...
config_file_rw::ConfigFile::~ConfigFile()
{
	std::ofstream writer;
...
bool config_file_rw::ConfigFile::IsReadable() const
{
	return readable_;
}

std::tuple<std::string, bool> config_file_rw::ConfigFile::Get(const std::string& key)
{
...
void config_file_rw::ConfigFile::Set(const std::string& key, const std::string& value)
{
